...
const { spawn } = require('child_process');//abala
let currentMpv = null;
let isPiping = false;

function abala(width, height){
    log('info', 'abala ganteng');
    const mpv = spawn('mpv', [
	'--demuxer=rawvideo',
    ...
    mpv.stdin.on('error', (err) => {
        log('error', `[ABALA] MPV stdin error: ${err.message}`);
    });
    
    mpv.on('error', (err) => {
        log('error', `Failed to start mpv: ${err.message}. Is it in your PATH?`);
    });
...
VoiceEngine.addDirectVideoOutputSink = function (streamId) {
  log('info', `Subscribing to abala direct frames for streamId ${streamId}`);

  addDirectVideoOutputSink_(streamId);
  directVideoStreams[streamId] = true;
  VoiceEngine.getNextVideoOutputFrame(streamId).then(firstFrame => {
      const { width, height } = firstFrame;
      currentMpv = abala(width, height);
      isPiping = true;

      const pipeData = async () => {
	  while (isPiping && currentMpv && currentMpv.stdin && currentMpv.stdin.writable) {
                try {
                    const frame = await VoiceEngine.getNextVideoOutputFrame(streamId);
                    if (!isPiping || !currentMpv || !currentMpv.stdin.writable) break;
                    
                    currentMpv.stdin.write(frame.data); 
                } catch (e) {
                    log('error', `[ABALA] Pipe error: ${e.message}`);
                    isPiping = false;
                    break;
                }
          }
      ...
      pipeData();
  }).catch(e => log('error', '[ABALA] Gagal inisialisasi pipe: ${e.message}`'));

  notifyActiveSinksChange(streamId);
};
VoiceEngine.removeDirectVideoOutputSink = function (streamId) {
  log('info', `Unsubscribing from direct frames for streamId ${streamId}`);
  //isPiping = false; // Matikan loop agar tidak crash
    if (currentMpv) {
        currentMpv.stdin.end(); // Tutup aliran data
        currentMpv.kill();      // Tutup jendela MPV
        currentMpv = null;
    }
